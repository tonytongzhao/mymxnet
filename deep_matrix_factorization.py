...
def mf(max_user, max_item, num_hidden):
    ...
    user=mx.sym.FullyConnected(data=user, name='user_fc1', num_hidden=num_hidden)
    user=mx.sym.Activation(data=user, name='user_relu1',act_type='relu')

    item=mx.sym.Embedding(data=item, input_dim=max_item, output_dim= num_hidden)
    item=mx.sym.FullyConnected(data=item, name='item_fc1', num_hidden=num_hidden)
    item=mx.sym.Activation(data=item, name='item_relu1',act_type='relu')
    pred=user*item
