...
class DataIter(mx.io.DataIter):
    def __init__(self, data, batch_size, user2item, item2user):
...
def get_data(data, split, batch_size, user2item, item2user):
    tr=random.sample(data, int(split*len(data)))
    te=list(set(data)-set(tr))

    return (DataIter(tr, batch_size, user2item, item2user), DataIter(te, batch_size, user2item, item2user))

def train(args,data_path, data, val, split, network, batch_size, num_epoch, user2item, item2user, learning_rate, logname):
    ''' 
    ...
    '''
    network=mx.mod.Module(network, data_names=('user', 'item',), label_names=('rating',), context=mx.gpu())
    ...
    if not args.val:
        train, val_iter= get_data(data, split, batch_size, user2item, item2user)
    else:
...
if __name__=='__main__':
    ...
    parser.add_argument('-nlayer', help='num of mlp layers', dest='num_layer', default=[100])
    ...
    num_hidden=[int(i) for i in args.num_layer.split(',')]
    num_layer=len(num_hidden)-1
    split=float(args.split)
    net=model.get_ncf(batch_size, num_embed, num_hidden, num_layer, len(user_dict), len(item_dict), float(args.dropout))
    logname="_embed_"+str(args.num_embed)+'_hidden_'+str(args.num_layer)+"_eta_"+str(args.learning_rate)+"_batch_size_"+str(args.batch_size)
    train(args, args.fi,data, val, split, net, batch_size, num_epoch, user2item, item2user, learning_rate, logname)
