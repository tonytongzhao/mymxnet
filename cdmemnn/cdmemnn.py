...
def train(args,data_path, data, val, split, network, batch_size, num_epoch, user2item, item2user, upass, ipass, learning_rate, logname):
    ...
    network.fit(train, eval_data=val_iter, eval_metric=RMSE, optimizer_params={'learning_rate':learning_rate, 'momentum':0.85}, num_epoch=num_epoch, batch_end_callback=mx.callback.Speedometer(batch_size, 5000))
    '''
